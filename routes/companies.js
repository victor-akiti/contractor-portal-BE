...
Router.get("/approvals/all", authenticate, checkIfUserHasPermissions(["Admin", "VRM", "CO", "Amni Staff", "GMD", "GM", "HOD"]), fetchAllApprovalData)
...
Router.post("/vendor/create", authenticate, checkIfUserHasPermissions(["Vendor"]), createVendor)
Router.put("/vendor/update", authenticate, checkIfUserHasPermissions(["Vendor"]), updateVendor)
Router.put("/vendor/submit", authenticate, checkIfUserHasPermissions(["Vendor"]), submitApplication)
Router.put("/certificates/:certificateID", authenticate, checkIfUserHasPermissions(["Vendor"]), updateCertificate)
Router.put("/job-categories/:id", authenticate, updateCompanyJobCategoriesList)

//Company settings and portal admin routes
Router.get("/settings/:vendorID", authenticate, checkIfUserHasPermissions(["Vendor", "Admin"]), getAllSettings)
Router.put("/portal-admin/update/:vendorID", authenticate, checkIfUserHasPermissions(["Vendor", "Admin"]), updatePortalAdministratorProfile)
Router.post("/portal-admin/replace/:vendorID", authenticate, checkIfUserHasPermissions(["Vendor", "Admin"]), requestNewPortalAdministrator)
